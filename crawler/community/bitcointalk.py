...
class bitcointalk(crawler):
    ...
    def crawlingPage(self, pageno):
        ...
        try:
            print(str(datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + " | pageno : " + str(pageno));
            ...
            for postinfo in postinfolist:
                ...
                try:
                    ...
                    print('post problem')
                    result["post_problems"] = postlist # if individual posts has problem, save as another list in json

            result["posts"] = postlist;

            f = open("output1/bitcointalk" + "_" + str(pageno) + ".json", "wb");
            ...
            # when whole page has problem
            print('page problem')
            f = open("output1/bitcointalk_pageproblems" + "_" + str(pageno) + ".json", "wb");
            f.close();
            crawlingPage(self, pageno + 1)
    ...
    def __parsePost(self, address, replycount):
        ...
        for tr in trlist:
            ...
            author_info = tr.find("td", {"class": "poster_info"}) # Author
            author = self.__removeTag(author_info.prettify().split("\n"))
